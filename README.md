...
A token, used to certify a user's identity, is generated by a server and sent to the client. Then client then sends the token back to the server for every subsequent request. This provides a level of security, but tokens can be stolen and must therefore not be your only line of defense.

- [ ] What steps can you take in your web apps to keep your data secure?

Use TLS(still sometimes called SSL) and HTTPS secure protocol
...
A web server is, very simply, a computer that hosts one or more websites on the internet by storing web server software and a website's component files. It is connected to the Internet and supports physical data interchange with other devices connected to the web. The software controls how web users access hosted files; when a browser sends a request to the server, the software on the server accepts the request, finds the requested file, and sends it back to the end-user's device (if the requested file is not found, a 404 Not Found client error response code is returned instead).
...
Both PUT and POST can create and update resources; the key difference is that contrary to POST, multiple identical PUT requests should have the same effect as a single request. Consequently PUT is a "replace" operation, which one could argue is not "update".
